...
class StochasticStrategy(BaseStrategy):
    ...
    description = 'Stochastic %K/%D crossover in oversold/overbought zones with trend filter.'
    version     = '1.0.0'
    ...
    def generate_signal(self, df: pd.DataFrame, symbol: str, **kwargs) -> Signal:
        ...
        if bool(self._p('stoch_rsi_mode')):
            delta = close.diff()
            gain  = delta.clip(lower=0).ewm(com=k_p-1, adjust=False).mean()
            loss  = (-delta.clip(upper=0)).ewm(com=k_p-1, adjust=False).mean()
            rsi   = 100 - 100/(1+gain/loss.replace(0, np.nan))
            raw_k = (rsi - rsi.rolling(k_p).min()) / \
                    (rsi.rolling(k_p).max() - rsi.rolling(k_p).min()).replace(0, np.nan) * 100
        else:
            ll    = low.rolling(k_p).min()
            hh    = high.rolling(k_p).max()
        ...
        trend_up   = sma200 is None or price > float(sma200.iloc[-1])
        trend_down = sma200 is None or price < float(sma200.iloc[-1])
        ...
        if cross_up and k_now < oversold and trend_up:
        ...
        if cross_down and k_now > overbought and trend_down:
            sl = price + atr * atr_mult
        ...
        return Signal(action='hold', symbol=symbol, strength=0.0,
                     reason=f"Stochastic neutral — K={k_now:.1f} D={d_now:.1f}",
                     indicators=indicators)
